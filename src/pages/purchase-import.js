...
export default function PurchaseImport() {
  ...
  const addPages = async (files) => {
    ...
    setPages(prev => {
      ...
      setMessage(`✓ ${newPages.length} page${newPages.length > 1 ? 's' : ''} added. Total: ${updated.length} page${updated.length > 1 ? 's' : ''}.`)
      return updated
    })
  }
  ...
  return (
    <StoreLayout>
      <div style={{ ...s.page, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, padding: '80px 24px', textAlign: 'center', minHeight: '60vh' }}>
        <div style={{ fontSize: 48 }}>🚫</div>
        <h2 style={{ color: '#0f3460', fontWeight: 900, fontSize: 22 }}>AI Scanner is disabled</h2>
        <p style={{ color: '#64748b', fontSize: 14, maxWidth: 400, margin: '0 auto' }}>
          The AI scan feature has been deactivated. Please use the standard protocols to convert and download your distributor bills.
        </p>
        <a href="/bill-converter" style={{ padding: '12px 24px', background: '#0e9090', color: 'white', textDecoration: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13, marginTop: 12 }}>
          Go to Bill Converter
        </a>
      </div>
    </StoreLayout>
  )

    ...
    <div style={s.page}>
      ...
      <div style={s.body}>
        {user && (
          <div style={s.tabs}>
            ...
            <button style={activeTab === 'appointments' ? s.activeTab : s.tab} onClick={() => setActiveTab('appointments')}>
              📅 Appointments
              {appointments.length > 0 && <span style={s.tabBadge}>{appointments.length}</span>}
            </button>
          </div>
        )}
        ...
        {activeTab === 'appointments' && (
          <>
            {appointments.length === 0 && <div style={s.empty}>No appointments yet. Apply for jobs to get started! 🚀</div>}
            {appointments.map(appt => (
              <AppointmentCard key={appt.id} appointment={appt} onAction={handleAppointmentAction} isUpcoming={isUpcoming(appt.appointment_date)} formatDate={formatDate} formatTime={formatTime} />
            ))}
          </>
        )}
      </div>
...
function AppointmentCard({ appointment, onAction, isUpcoming, formatDate, formatTime }) {
  ...
  return (
    <div style={{ ...s.apptCard, ...(isUpcoming && appointment.status === 'confirmed' ? s.upcomingCard : {}) }}>
      {isUpcoming && appointment.status === 'confirmed' && (
        <div style={s.reminderBanner}>🔔 This appointment is today or tomorrow!</div>
      )}
      <h3 style={s.apptTitle}>{job?.title}</h3>
...
const s = {
  ...
  upcomingCard: { border: '2px solid #f59e0b' },
  ...
  dtBox: { background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 12, margin: '12px 0', display: 'flex', flexDirection: 'column', gap: 8 },

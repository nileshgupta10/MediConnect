...
export default function AdminPage() {
  ...
  const getPerformance = async (userId, type) => {
    if (type === 'store') {
      ...
      return { jobsPosted: jobsRes.data?.length || 0, jobsClosed: jobsRes.data?.filter(j => j.status === 'closed').length || 0, appointmentsConfirmed: apptsRes.data?.filter(a => a.status === 'confirmed').length || 0 }
    } else {
      ...
      return { jobsApplied: appsRes.data?.length || 0, appointmentsConfirmed: apptsRes.data?.filter(a => a.status === 'confirmed').length || 0 }
    }
  }
...
function PharmacistCard({ item, status, onApprove, onReject, onViewLicense, getPerformance }) {
  ...
  const loadPerf = async () => {
    ...
    setLoadingPerf(true)
    setPerf(await getPerformance(item.user_id, 'pharmacist'))
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={styles.cardActions}>
        {item.license_url && <button style={styles.licenseBtn} onClick={onViewLicense}>📄 View License</button>}
        <button style={styles.perfBtn} onClick={loadPerf} disabled={loadingPerf}>{loadingPerf ? 'Loading…' : perf ? 'Hide Stats' : '📊 View Stats'}</button>
      </div>
      {perf && <div style={styles.perfBox}><p style={styles.perfItem}>📋 Jobs Applied: <b>{perf.jobsApplied}</b></p><p style={styles.perfItem}>✓ Appointments Confirmed: <b>{perf.appointmentsConfirmed}</b></p></div>}
...
function StoreCard({ item, status, onApprove, onReject, getPerformance }) {
  ...
  const loadPerf = async () => {
    if (perf) { setPerf(null); return }
    setLoadingPerf(true)
    setPerf(await getPerformance(item.user_id, 'store'))
    setLoadingPerf(false)
  }
  return (
    <div style={styles.card}>

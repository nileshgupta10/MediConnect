// FIX 2 of 3 — patwari.js (mapCsvRows function only — the rest is unchanged)
//
// BUG: discountPer = cleanNum(row.cdper || 0) → produced 7.00
// CAUSE: Patwari's CSV cdper column = 2× the printed invoice DIS value.
//        grsamt in the CSV is already computed at the full 7% discount (qty*PTR*(1-7%))
//        so grsamt is correct. But CARE's DISCOUNT field should show 3.5 (what's on paper).
// FIX:  discountPer = cleanNum(row.cdper || 0) / 2  → 3.50
//
// Also added:
// grossAmtPreDisc = qty * rate  (qty × PTR = pre-discount gross, used for GROS_AMT in CARE)
// discAmt = grossAmtPreDisc - grossAmt  (derived, so GROS_AMT - DISC_AMT = taxable always balances)
//
// NOTE: This halving of cdper is SPECIFIC TO PATWARI. Other distributors use cdper directly.
...
function mapCsvRows(rows) {
  const items = rows.map((row) => {
    ...
    // grsamt = post-CD-discount gross amount (qty × PTR × (1 - cdper/100))
    // Patwari's cdper is 7.00 but invoice DIS column shows 3.50 (it's doubled in their system)
    const grossAmt = cleanNum(row.grsamt)         // taxable base — correct as-is

    // ✅ FIX: cdper in Patwari CSV is 2× the invoice's DIS column value
    // Divide by 2 so CARE stores the correct visible discount (3.5 not 7.0)
    const discountPer = cleanNum(row.cdper || 0) / 2

    // Pre-discount gross = qty × PTR (what CARE shows as GROS_AMT)
    const grossAmtPreDisc = qty * rate

    // Derive DISC_AMT so that GROS_AMT - DISC_AMT = taxable (always balances in CARE)
    const discAmt = grossAmtPreDisc - grossAmt
    ...
    return {
      ...
      rate,
      rawRate: rate,
      grossAmt: grossAmtPreDisc,  // ✅ FIX: GROS_AMT = qty*PTR (pre-disc), not grsamt
      ...
      discountPer,               // ✅ FIX: cdper/2 = 3.5 (matches invoice DIS column)
      discAmt,                   // ✅ FIX: derived from GROS_AMT - taxable (balances)
      taxable,                   // grsamt IS the taxable base (unchanged)
  ...
  // Invoice-total reconciliation (adjust first item by rounding diff if < ₹1)
  if (items.length > 0) {

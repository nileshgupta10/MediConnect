...
export default function AdminLayout({ children }) {
  ...
  const section = router.query.section || 'pharmacists'
  ...
  const nl = (sec) => section === sec ? s.activeLink : s.link
  ...
  return (
    <div style={s.wrap}>
      <nav style={s.nav}>
        ...
        <a href="/admin?section=pharmacists" style={nl('pharmacists')}>Pharmacists</a>
        <a href="/admin?section=stores" style={nl('stores')}>Stores</a>
        <a href="/admin?section=jobs" style={nl('jobs')}>Jobs</a>
...
const s = {
  ...
  adminBadge: { background: '#0f3460', color: 'white', fontSize: 10, fontWeight: 800, padding: '2px 6px', borderRadius: 6 },
